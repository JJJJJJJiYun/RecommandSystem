    ...
    def user_cf(self):
        ...
        for user1, euclidean_distances in users_euclidean_distance_dict.items():
            ...
            for item, users in self.item_users_dict.items():
                ...
                total_score += score
            user_total_score_dict[user1] = total_score
        ...
        # 化成占比用于合并
        for user, items_scores in self.user_cf_users_items_interest_dict.items():
            for i, (item, score) in enumerate(items_scores):
                self.user_cf_users_items_interest_dict[user][i] = (item, score / user_total_score_dict[user])
        print("normalizatoin finished, total score:", user_total_score_dict)
        print(self.user_cf_users_items_interest_dict)
    ...
    def item_cf(self):
        ...
        for user, items in self.user_items_dict.items():
            ...
            for item1, _ in self.item_users_dict.items():
                ...
                total_score += score
            user_total_score_dict[user] = total_score
        ...
        # 化成占比用于合并
        for user, items_scores in self.item_cf_users_items_interest_dict.items():
            for i, (item, score) in enumerate(items_scores):
                self.item_cf_users_items_interest_dict[user][i] = (item, score / user_total_score_dict[user])
        print("normalizatoin finished, total score:", user_total_score_dict)
        print(self.item_cf_users_items_interest_dict)
